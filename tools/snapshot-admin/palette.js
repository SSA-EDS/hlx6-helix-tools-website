...
async function init() {
  ...
  REVIEWS_LINK.href = customReviewHost ? `https://${customReviewHost}${PATHNAME}` : `https://${SNAPSHOT}--main--${REPO}--${OWNER}.{{BASE_DOMAIN}}.reviews${PATHNAME}`;
  ADMIN_LINK.href = `/tools/snapshot-admin/index.html?snapshot=https://main--${REPO}--${OWNER}.{{BASE_DOMAIN}}.page/.snapshots/${SNAPSHOT}/.manifest.json`;
...
REVIEW_APPROVE.addEventListener('click', async () => {
  ...
  window.parent.location.href = customLiveHost ? `https://${customLiveHost}${PATHNAME}` : `https://main--${REPO}--${OWNER}.{{BASE_DOMAIN}}.live${PATHNAME}`;
});
