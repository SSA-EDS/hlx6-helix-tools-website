...
describe('aem-admin.js — H6 URL contract', () => {
  ...
  describe('admin.config(coords) URLs', () => {
    it('site-scoped URL is /{org}/sites/{site}/config.json', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/config.json',
      );
    });
    ...
    it('org-only URL is /{org}/config.json', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/config.json',
      );
    });
    ...
    it('profile-scoped URL is /{org}/profiles/{profile}/config.json', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/profiles/p/config.json',
      );
    });
    ...
    it('select from site root descends into /{org}/sites/{site}/config/', async () => {
      ...
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/config/robots.txt');
    });
  ...
  describe('admin.status(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/status', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/status',
      );
    });

    it('.get(path) appends to the base URL', async () => {
      await admin.status({ org: 'adobe', site: 'x' }).get('/en/index');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/status/en/index');
    });
    ...
    it('.get(path, { params }) appends query string', async () => {
      ...
      assert.equal(
        calls[0].url,
        'https://api.entmseds.live/adobe/sites/x/status/page?editUrl=auto',
      );
    });
  ...
  describe('admin.preview(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/preview', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/preview',
      );
    });

    it('.get(path) GETs the preview status', async () => {
      await admin.preview({ org: 'adobe', site: 'x' }).get('/en/index');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/preview/en/index');
    });
  ...
  describe('admin.live(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/live', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/live',
      );
    });

    it('.get/.update/.remove hit /{org}/sites/{site}/live/{path}', async () => {
      await admin.live({ org: 'adobe', site: 'x' }).get('/en/index');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/live/en/index');
    });
  });
  ...
  describe('admin.code(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/code', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/code',
      );
    });

    it('.get/.update/.remove hit /{org}/sites/{site}/code/{path}', async () => {
      await admin.code({ org: 'adobe', site: 'x' }).get('/scripts/scripts.js');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/code/scripts/scripts.js');
    });
  });
  ...
  describe('admin.psi(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/psi', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/psi',
      );
    });

    it('.get() GETs the psi endpoint', async () => {
      await admin.psi({ org: 'adobe', site: 'x' }).get('');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/psi');
  ...
  describe('admin.log(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/log', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/log',
      );
    });

    it('.get(path) GETs logs', async () => {
      await admin.log({ org: 'adobe', site: 'x' }).get('');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/log');
      assert.equal(calls[0].init.method, 'GET');
    });
  ...
  describe('admin.index(coords) URLs', () => {
    it('.update("/*", body) hits /{org}/sites/{site}/index/*', async () => {
      ...
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/index/*');
    });
  });
  ...
  describe('admin.sitemap(coords) URLs', () => {
    it('.update("/sitemap.xml") hits /{org}/sites/{site}/sitemap/sitemap.xml', async () => {
      ...
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/sitemap/sitemap.xml');
    });
  });
  ...
  describe('admin.job(coords) URLs', () => {
    it('.get("topic/name") hits /{org}/sites/{site}/job/topic/name', async () => {
      ...
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/job/index/job-123');
    });
  });
  ...
  describe('admin.snapshot(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/snapshot', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/snapshot',
      );
    });

    it('.get("") GETs the snapshot list', async () => {
      await admin.snapshot({ org: 'adobe', site: 'x' }).get('');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/snapshot');
      assert.equal(calls[0].init.method, 'GET');
    });

    it('.get("name") GETs the named snapshot manifest', async () => {
      await admin.snapshot({ org: 'adobe', site: 'x' }).get('my-snapshot');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/snapshot/my-snapshot');
      assert.equal(calls[0].init.method, 'GET');
    });
  ...
  describe('admin.sidekick(coords) URLs', () => {
    it('.url is /{org}/sites/{site}/sidekick', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/sidekick',
      );
    });

    it('.get("config.json") GETs the sidekick config', async () => {
      await admin.sidekick({ org: 'adobe', site: 'x' }).get('config.json');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/sidekick/config.json');
      assert.equal(calls[0].init.method, 'GET');
    });
  ...
  describe('admin.medialog(coords) URLs', () => {
    it('.get("") GETs /{org}/sites/{site}/medialog', async () => {
      ...
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/medialog');
      assert.equal(calls[0].init.method, 'GET');
    });
    ...
    it('exposes .url equal to the base operation URL', () => {
      assert.equal(
        ...
        'https://api.entmseds.live/adobe/sites/x/medialog',
      );
    });
  });

  describe('admin.raw() H6 URLs', () => {
    it('/path resolves against https://api.entmseds.live', async () => {
      await admin.raw('GET', '/adobe/sites/x/status');
      assert.equal(calls[0].url, 'https://api.entmseds.live/adobe/sites/x/status');
    });
  });
  ...
  describe('admin.suggestions(coords) H6 URLs', () => {
    it('org-only includes /{org}/config.json', () => {
      ...
      assert.ok(items.some(({ url }) => url === 'https://api.entmseds.live/adobe/config.json'));
    });

    it('org-only includes /{org}/sites.json and /{org}/profiles.json', () => {
      const items = admin.suggestions({ org: 'adobe' });
      assert.ok(items.some(({ url }) => url === 'https://api.entmseds.live/adobe/sites.json'));
      assert.ok(items.some(({ url }) => url === 'https://api.entmseds.live/adobe/profiles.json'));
    });

    it('with site includes /{org}/sites/{site}/config.json', () => {
      const items = admin.suggestions({ org: 'adobe', site: 'x' });
      assert.ok(items.some(({ url }) => url === 'https://api.entmseds.live/adobe/sites/x/config.json'));
    });

    it('with site includes status and preview URLs', () => {
      const items = admin.suggestions({ org: 'adobe', site: 'x' });
      assert.ok(items.some(({ url }) => url === 'https://api.entmseds.live/adobe/sites/x/status'));
      assert.ok(items.some(({ url }) => url === 'https://api.entmseds.live/adobe/sites/x/preview'));
    });
  ...
  describe('admin.coordsFromURL(url) H6 patterns', () => {
    it('parses org + site from /{org}/sites/{site}/config.json', () => {
      assert.deepEqual(
        admin.coordsFromURL('https://api.entmseds.live/adobe/sites/x/config.json'),
        { org: 'adobe', site: 'x' },
      );
    });
    ...
    it('parses org-only from /{org}/config.json', () => {
      assert.deepEqual(
        admin.coordsFromURL('https://api.entmseds.live/adobe/config.json'),
        { org: 'adobe', site: null },
      );
    });
    ...
    it('treats /{org}/sites.json as org-only (list, not a specific site)', () => {
      assert.deepEqual(
        admin.coordsFromURL('https://api.entmseds.live/adobe/sites.json'),
        { org: 'adobe', site: null },
      );
    });
    ...
    it('parses org + site from an operation URL', () => {
      assert.deepEqual(
        admin.coordsFromURL('https://api.entmseds.live/adobe/sites/x/status'),
        { org: 'adobe', site: 'x' },
      );
    });
    ...
    it('parses org + site from an operation URL with content path', () => {
      assert.deepEqual(
        admin.coordsFromURL('https://api.entmseds.live/adobe/sites/x/preview/en/index'),
        { org: 'adobe', site: 'x' },
      );
    });
    ...
    it('derived client coordsFromURL parses correctly', () => {
      ...
      assert.deepEqual(
        a.coordsFromURL('https://api.entmseds.live/adobe/sites/x/config.json'),
        { org: 'adobe', site: 'x' },
      );
    });
  });
